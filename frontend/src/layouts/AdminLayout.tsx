...
import { Activity, Key, Settings, LayoutDashboard, MessageSquare, Menu, X, Wand2 } from "lucide-react"
...
export default function AdminLayout() {
  ...
  const navs = [
    { name: tr.nav.dashboard, path: "/", icon: LayoutDashboard },
    { name: tr.nav.accounts, path: "/accounts", icon: Activity },
    { name: tr.nav.generator, path: "/generator", icon: Wand2 },
    { name: tr.nav.apiKeys, path: "/tokens", icon: Key },
    { name: tr.nav.test, path: "/test", icon: MessageSquare },
    { name: tr.nav.settings, path: "/settings", icon: Settings },
  ]
  ...
  return (
    <div className="flex min-h-screen w-full bg-background text-foreground transition-colors duration-300">
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-border/40">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <div className="text-[10px] text-muted-foreground font-medium tracking-widest uppercase leading-none">{tr.nav.subtitle}</div>
            </div>
          </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-16 flex items-center justify-between px-6 border-b border-border/40 bg-card/80 backdrop-blur-xl z-10 shadow-sm">
          {/* Left: mobile logo */}
          ...
          <div className="flex items-center gap-2">
            {(["en", "cn", "tr"] as const).map(l => (
              <button key={l} onClick={() => setLang(l)}
                className={`h-7 px-2.5 rounded-full border text-xs font-bold tracking-wide transition-colors ${lang === l ? "bg-primary text-primary-foreground border-primary" : "border-border/60 bg-muted/40 hover:bg-muted text-muted-foreground"}`}>
                {l === "en" ? "EN" : l === "cn" ? "中文" : "TR"}
              </button>
            ))}
            <button className="md:hidden text-muted-foreground hover:text-foreground transition-colors ml-1" onClick={() => setMobileOpen(true)}>
