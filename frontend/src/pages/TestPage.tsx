...
export default function TestPage() {
  ...
  const [messages, setMessages] = useState<Msg[]>([{ role: "user", content: "Hello" }])
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err?.detail?.error?.message || err?.error?.message || `HTTP ${res.status}`)
      }
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex justify-between items-center">
        ...
        <Button variant="outline" size="sm" onClick={fetchModels}><RefreshCw className="mr-2 h-4 w-4" /> {t.refreshModels}</Button>
      </div>

      <div className="flex gap-4 items-end flex-wrap">
        <div className="flex-1 min-w-[200px]">
          <label className="text-xs font-semibold mb-1.5 block">{t.model}</label>
        ...
        <div className="flex-1 min-w-[200px]">
          <label className="text-xs font-semibold mb-1.5 block">{t.messageContent}</label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="rounded-xl border bg-card/40">
          <div className="flex items-center gap-2 p-4 border-b bg-muted/10">
            ...
            <h3 className="font-semibold text-sm">{t.requestMessage}</h3>
          </div>
        ...
        <div className="rounded-xl border bg-card/40">
          <div className="flex items-center gap-2 p-4 border-b bg-muted/10">
            ...
            <h3 className="font-semibold text-sm">{t.response}</h3>
          ...
          <pre className="p-4 text-xs font-mono text-slate-300 bg-slate-950 min-h-[200px] max-h-[500px] overflow-auto whitespace-pre-wrap">
            {result ? result : <span className="text-muted-foreground">{t.responsePlaceholder}</span>}
          </pre>
        </div>
      </div>
    </div>
